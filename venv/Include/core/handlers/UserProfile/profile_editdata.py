...
async def process_callback_authbtn(callback_query: types.CallbackQuery):
    ...
    await bot.send_message(chat_id=callback_query.from_user.id, text=last_name_txt+edit_addon,reply_markup=markup)
...
async def group(callback_query: types.CallbackQuery, state: FSMContext):
    async with state.proxy() as data:
        ...
        data['fond'] = fond
        await bot.send_message(callback_query.from_user.id, city_where + edit_addon,reply_markup=await get_btns_param('city',data))
        await UserProfileEdit.next()
...
async def group(message: types.Message, state: FSMContext):
    if message.text == "0":
        await bot.send_message(message.from_user.id, date_example+edit_addon)
...
async def city(callback_query: types.CallbackQuery, state: FSMContext):
    async with state.proxy() as data:
        ...
        data['city'] = city
        await bot.send_message(callback_query.from_user.id, year_join, reply_markup=await get_btns_param('year',data))
        await UserProfileEdit.next()
...
async def daymonthenter(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        try:
            ...
            if "year" not in data:
                data['year'] = str(requestsApi.getUserData(message.from_user.id)['startdate']).split("-")[0]
            year = str(data['year'])
            date = year+"-"+month+"-"+day
...
@dp.message_handler(state=UserProfileEdit.EDIT_PHOTO, content_types=['photo','document'])
...
async def get_btns_param(param,data):
    curgroup = data['fond']
    data_list = []
    groups = requestsApi.getgroups()
    for i in range(0, len(groups)):
